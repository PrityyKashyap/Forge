...
| 9+ (replication) | **Correction, recorded post-implementation (Phase 9), not silently edited away**: this row originally planned two selectable ack policies, sync and async, benchmarked against each other. Only **async replication was actually built**: the leader acks a `PUT`/`DELETE` after its own WAL fsync completes, and forwards the record to followers afterward, never blocking the local write on any follower's ack. There is no sync/opt-in mode — **a write acknowledged by the leader can be lost if the leader crashes before replicating it to any follower** (see docs/FAILURE_MODEL.md §3, and Phase 11 Scenario A, which tests exactly this). BENCHMARKS.md §7.2 (E13) measures async replication's real, nonzero cost to the leader's own write latency as follower count grows — a genuinely surprising result investigated there, not asserted. |
